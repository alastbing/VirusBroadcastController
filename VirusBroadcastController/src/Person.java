...
public class Person extends Point {
    ...
    public boolean wantMove() {
        return MathUtil.stdGaussian(sig, constants.getU()) > 0;
    }
    ...
    public void beInfected() {
        state = State.SHADOW;
        infectedTime = myPanel.getWorldTime();
    }
...
//        if (getX() > Constants.CITY_WIDTH || getX() < 0) {
        if (getX() > city.getCenterX() * 2 || getX() < 0) {
            moveTarget = null;
            if (udX > 0) {
                udX = -udX;
            }
...
//        if (getY() > Constants.CITY_HEIGHT || getY() < 0) {
        if (getY() > city.getCenterY() * 2 || getY() < 0) {
            moveTarget = null;
            if (udY > 0) {
                udY = -udY;
            }
    ...
    public void update() {
        ...
        if (state == State.CONFIRMED
                && myPanel.getWorldTime() - confirmedTime >= constants.getHospitalReceiveTime()) {
        ...
        //处理病死者
        if ((state == State.CONFIRMED || state == State.FREEZE) && myPanel.getWorldTime() >= dieMoment && dieMoment > 0) {
            state = State.DEATH;//患者死亡
            hospital.returnBed(useBed);//归还床位
        }

        //增加一个正态分布用于潜伏期内随机发病时间
        double stdRnShadowtime = MathUtil.stdGaussian(25, constants.getShadowTime() / 2);
        //处理发病的潜伏期感染者
        if (myPanel.getWorldTime() - infectedTime > stdRnShadowtime && state == State.SHADOW) {
        ...
        for (Person person : people) {
            ...
            if (random < constants.getBroadRate() && distance(person) < SAFE_DIST) {
                this.beInfected();
                break;
            }
        }
